...
double SiPM_Ratio[32] = {1., 0.938931, 1.00446, 0.934524, 0.935622, 0.93, 1.08922, 1.03745, 0.944903, 0.956835, 1.01205, 0.771186, 1.14286, 0.771605, 1.2283, 0.751366, 1.17293, 0.752852, 1.18605, 0.731383, 1.44178, 0.696793, 1.41329, 0.77551, 1.33226, 0.666667, 1.4494, 0.910714, 1.125, 0.595483, 1.61826};
double SiPM_Ratio_error[32] = {.01, 0.116936, 0.109118, 0.0370326, 0.0447589, 0.0664147, 0.0519086, 0.0875793, 0.0326701, 0.0313826, 0.0572764, 0.0334097, 0.0897684, 0.0191097, 0.0369718, 0.070202, 0.0688351, 0.0173146, 0.0452497, 0.0282514, 0.0772691, 0.11697, 0.0449403, 0.0853006, 0.0518134, 0.0461705, 0.042968, 0.0209688, 0.0388808, 0.0609941, 0.129608};

double * GetRatio(TProfile *h1_tile_response[], int ch_1, int ch_2){
  ...
  int max_bin_odd = h1_tile_response[odd]->GetMaximumBin();
  int max_bin_even = h1_tile_response[even]->GetMaximumBin();

  double max_odd[5];
  double max_even[5];
  for (int i = 0; i < 5; i++){
    max_odd[i] = h1_tile_response[odd]->GetBinContent(max_bin_odd - 2 +i);
    max_even[i] = h1_tile_response[even]->GetBinContent(max_bin_even - 2 +i);
    cout<<"odd/even: "<<max_odd[i]<<"/"<<max_even[i]<<endl;
  }
  ...
  for (int i = 0; i < 5; i++){
    avg_odd += max_odd[i]/5.;
    avg_even += max_even[i]/5.;
    ms_odd += max_odd[i]*max_odd[i]/5.;
    ms_even += max_even[i]*max_even[i]/5.;
  }
  double std_even = abs(ms_even - avg_even*avg_even);
  double std_odd = abs(ms_odd - avg_odd*avg_odd);
  cout<<"Even: "<<avg_even<<" +/- "<<std_even<<endl;
  cout<<"odd: "<<avg_odd<<" +/- "<<std_odd<<endl;

  ratio[0] = avg_odd/avg_even;
  ratio[1] = ratio[0]*sqrt(TMath::Power(std_odd/avg_odd,2)+TMath::Power(std_even/avg_even,2));
  cout<<"odd: "<<ratio[0]<<" +/- "<<ratio[1]<<endl;

  return ratio;
}


int GetCrosstalkDistance(TH1D *h_bin_response[], double max_x[], double bag[], double end[], TProfile *h1_tile_response[], int ch_1, int ch_2){
  ...
  for ( int i = 0; i < size; i++){
    ...
    for ( int j = 1; j <= h1_tile_response[channels[i]]->GetNbinsX(); j++){
      if(h1_tile_response[channels[i]]->GetBinCenter(j) < 3) continue;
      if(h1_tile_response[channels[i]]->GetBinCenter(j) < 4 && channels[i] != 1) continue;

      v_ref =  h1_tile_response[channels[i]]->GetBinContent(j);
      int max_i = 0;
      double max = 0;
      for (int k = 0; k < 16;k++){
        if (channels[k] == channels[i]) continue;
        double v = h1_tile_response[channels[k]]->GetBinContent(j);
        if (v > max){
          max = v;
          max_i = k;
        }
      ...
      if (v_ref > max) {
        if(ct[channels[i]] == 0){
          bag[channels[i]] = h1_tile_response[channels[i]]->GetBinCenter(j);
          ct[channels[i]]++;
        }
        ref[channels[i]] += v_ref;
        binlength[i] = binlength[i] + 1.;
      }
      else if(ct[channels[i]]==1){
        end[channels[i]] = h1_tile_response[channels[i]]->GetBinCenter(j - 1);
        ct[channels[i]]++;
      }
    }
  }
  for (int i = 0; i < size; i++){
    max_x[channels[i]] = (bag[channels[i]] + end[channels[i]])/2.;
  }
  int nbins;
  ...
  for (int i = 0; i < size;i++){
    ...
    for (int b = 1; b <= nbins; b++){
      ...
      h_bin_response[channels[i]]->Fill(h1_tile_response[channels[i]]->GetBinCenter(b), binlength[i]*(h1_tile_response[channels[i]]->GetBinContent(b)/ref[channels[i]]));
    }
...
void GetFileName_ratio(std::vector<std::string> &filenames, const std::string data_dir, std::string test_type, int sector = 0, int all_runs = 0, int ch1 = 0, int ch2 = 0, bool debug = false){
