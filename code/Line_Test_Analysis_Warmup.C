...
void Analyze(std::vector<std::string> &filenames, const int sector, const std::string data_dir, const std::string save_dir_raw, const std::string save_dir_plot, const std::string save_dir_root, int ch1, int ch2, bool debug = false){
  ...
  TH1D *h_bin_response[22];
  TH1D *hp_ratio = new TH1D("h_ratio","",15, 0.5, 15.5);
  TProfile *h1_tile_response_long[22];
  ...
  for (int ff = 0; ff < size; ff++){
    ...
    vector<std::array<float, 22>> dark_current;
    vector<std::array<float, 22>> temp_b_dark;
    vector<std::array<float, 22>> temp_c_dark;
...
//////////////////////
  ...
  TH1D *h_tile_cross_near = new TH1D("h_tile_cross_near","", 22, -0.1, 0.3);
  TH1D *h_tile_cross_far = new TH1D("h_tile_cross_far","", 22, -0.1, 0.3);
...
////////////
  ...
  TProfile *hp_crosstalk_distance_tile[22];
  for (int i = 0; i < 22; i++){
...
int Line_Test_Analysis_Warmup(const std::string &config_file= "line_config.config")
{
  bool debug = false;
