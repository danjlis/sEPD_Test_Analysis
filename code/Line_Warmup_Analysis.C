...
void Analyze(std::vector<std::string> &filenames, const int sector, const std::string data_dir, const std::string save_dir_raw, const std::string save_dir_plot, const std::string save_dir_root, int ch1, int ch2, bool debug = false){
    ...
    TH1D *h_bin_response[NTILE];
    TH1D *hp_ratio = new TH1D("h_ratio","",15, 0.5, 15.5);
    TProfile *h1_tile_response_long[NTILE];
    ...
    for (int ff = 0; ff < size; ff++){
        ...
        vector<std::array<float, NTILE>> dark_current;
        vector<std::array<float, NTILE>> temp_b_dark;
        vector<std::array<float, NTILE>> temp_c_dark;
        ...
        if (oo > 2){
            if (ch_1 == nWarm || ch_2 == nWarm) drawText("Tile 1 and Even Tiles",xPos,yPos-2*dy2,0, 1, fontSize, fontType);
        ...
        if (oo > 2){
            if (ch_1 == nWarm || ch_2 == nWarm) c_full->SetName("c_full_even");
        ...
        if ((ch_1 == nWarm || ch_2 == nWarm) && (ch_1 == 1 || ch_2 == 1)) {
        ...
        if (oo > 2){
          if (ch_1 == nWarm || ch_2 == nWarm) c_full->SetName("c_full_even_norm");
        ...
        int write = 0; //GetCrosstalkDistance(h1_bin_response_temp, max_x,bag,end, h1_tile_response_norm, ch_1, ch_2);
        for (int i = 0; i < NTILE; i++){
          int ns = 0;
          int oo = 0;
          for (std::vector<int>::iterator it = channels.begin(); it != channels.end(); ++it){
    ...
    TH1D *h_tile_cross_near = new TH1D("h_tile_cross_near","", NTILE, -0.1, 0.3);
    TH1D *h_tile_cross_far = new TH1D("h_tile_cross_far","", NTILE, -0.1, 0.3);
    ...
    TProfile *hp_crosstalk_distance_tile[NTILE];
    for (int i = 0; i < NTILE; i++){
    ...
    c9->SaveAs(Form("%s/GainComparison_%s.pdf", save_dir_plot.c_str(), sector_addon_n));
    c9->SaveAs(Form("%s/GainComparison_%s.png", save_dir_plot.c_str(), sector_addon_n));
...
int Line_Warmup_Analysis(const std::string &config_file= "line_config.config")  {
    bool debug = true;
