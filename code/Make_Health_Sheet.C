...
void Analyze(const int sector, const std::string data_dir, const std::string save_dir_raw, const std::string save_dir_plot, const std::string save_dir_root, bool debug = true, const int save_me_int = -1){
  ...
  top_name->Draw();
  TPad *top_mid = new TPad("top_mid", "",0.33, 0.6, 0.66, 0.9);
  top_mid->Draw();
  TPad *top_left = new TPad("top_left", "",0.0, 0.6, 0.33, 0.9);
  top_left->Draw();
  TPad *mid_mid = new TPad("mid_mid", "",0.33, 0.3, .66, 0.6);
  mid_mid->Draw();
  TPad *mid_left = new TPad("mid_left", "",0, 0.3, 0.33, 0.6);
  mid_left->Draw();
  TPad *bot_mid = new TPad("bot_mid", "",0.33, 0, .66, 0.3);
  bot_mid->Draw();
  TPad *bot_left = new TPad("bot_left", "",0, 0, 0.33, 0.3);
  bot_left->Draw();
  TPad *top_right = new TPad("top_right", "",0.66, .6, 1., 0.9);
  top_right->Draw();
  TPad *mid_right = new TPad("mid_right", "",0.66, .3, 1, 0.6);
  mid_right->Draw();
  TPad *bot_right = new TPad("bot_right", "",0.66, 0, 1, 0.3);
  bot_right->Draw();
